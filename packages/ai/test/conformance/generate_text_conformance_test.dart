  ...
  group('generateText conformance', () {
    ...
    group('providerMetadata', () {
      test('providerMetadata from model is passed through to result', () async {
        final model = FakeTextModel(
          'done',
          providerMetadata: const {'anthropic': {'cacheReadInputTokens': 5}},
        );
        final result = await generateText(model: model, prompt: 'hi');
        expect(result.providerMetadata, isNotNull);
        expect(
          (result.providerMetadata!['anthropic'] as Map?)?['cacheReadInputTokens'],
          5,
        );
      });
    });
    ...
    group('callbacks', () {
      test('onFinish is called with text, steps, usage, finishReason', () async {
      ...
      test('experimentalOnStart is called once with model and messages', () async {
    ...
    group('prepareStep', () {
      ...
      test('prepareStep can set activeTools to restrict which tools are exposed',
          () async {
    ...
    group('stopConditions', () {
      test('stopAfterMaxCalls(1) stops after one step even with tools', () async {
        final model = FakeMultiStepModel([
          LanguageModelV3GenerateResult(
            content: [
              const LanguageModelV3ToolCallPart(
