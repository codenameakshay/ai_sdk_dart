...
void main() {
  ...
  group('generateText structured output system instruction', () {
    test(
      'system is combined with object/array/choice/json schema text',
      () async {
        // object
        ...
        expect(
          objModel.capturedOptions.single.prompt.system,
          contains('be terse'),
        );
        ...
        expect(
          arrModel.capturedOptions.single.prompt.system,
          contains('arr-sys'),
        );
  ...
  group('generateText tool execution errors', () {
    ...
    test('throwing executor produces an error tool result', () async {
      final model = FakeMultiStepModel([
        ...
        const LanguageModelV4GenerateResult(
          content: [LanguageModelV4TextPart(text: 'recovered')],
    ...
    test('tool with no executor produces an error tool result', () async {
      final model = FakeMultiStepModel([
        ...
        const LanguageModelV4GenerateResult(
          content: [LanguageModelV4TextPart(text: 'after')],
          finishReason: LanguageModelV4FinishReason.stop,
        ),
      ]);
      final result = await generateText(
      ...
      final result = await generateText(
        ...
        tools: {
          'stream': tool<Map<String, dynamic>, Object?>(
            inputSchema: objectSchema(),
            execute: (_, _) async => Stream.fromIterable(['first', 'last']),
          ),
        },
      );
      final output =
    ...
    test(
      'onToolCallStart / onToolCallFinish fire (success and failure)',
      () async {
        var startCount = 0;
        final finishes = <bool>[];
        final model = FakeMultiStepModel([
  ...
  group('generateText tool approval flow', () {
    Tool<Map<String, dynamic>, Object?> approvalTool({required bool needs}) {
      return tool<Map<String, dynamic>, Object?>(
        inputSchema: objectSchema(),
        execute: (_, _) async => 'executed',
        needsApproval: (_, _) async => needs,
      );
    }

    test(
      'tool requiring approval with no response emits an approval request',
      () async {
        final model = FakeMultiStepModel([
          const LanguageModelV4GenerateResult(
            content: [
    ...
    test(
      'conditional approval executes when the evaluator returns false',
      () async {
        final model = FakeMultiStepModel([
          const LanguageModelV4GenerateResult(
            content: [
        ...
        final result = await generateText(
          ...
          prompt: 'go',
          maxSteps: 3,
          tools: {'safe': approvalTool(needs: false)},
        );

        expect(result.toolApprovalRequests, isEmpty);
    ...
    test('denied tool returns an error result with the reason', () async {
      ...
      final toolResult = result.steps.first.toolResults.single;
      expect(toolResult.isError, isTrue);
      expect((toolResult.output as ToolResultOutputText).text, 'denied!');
    });
  });
